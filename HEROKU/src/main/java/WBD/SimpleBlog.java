...
public class SimpleBlog {
	...
	public boolean addUser(@WebParam(name = "email") String email, @WebParam(name = "password") String password, @WebParam(name = "name") String name, @WebParam(name="role") String role) {
		try {			
			JSONObject json = new JSONObject(readUrl("https://scorching-inferno-6707.firebaseio.com/user/.json"));
			int counter = json.getInt("counter") + 1;
			Firebase firebase = new Firebase("https://scorching-inferno-6707.firebaseio.com/user/");
			...
			while(keys.hasNext()) {
				...
				if(!key.equals("counter")){
					...
					if(child.getString("email").equals(email)){
						Firebase base = new Firebase("https://scorching-inferno-6707.firebaseio.com/user/");						
	...
	public boolean addPost(@WebParam(name = "judul") String judul, @WebParam(name = "tanggal") String tanggal, @WebParam(name = "konten") String konten) {
		try {			
			JSONObject json = new JSONObject(readUrl("https://scorching-inferno-6707.firebaseio.com/post/.json"));
			int counter = json.getInt("counter") + 1;
			Firebase firebase = new Firebase("https://scorching-inferno-6707.firebaseio.com/post/");
			...
			while(keys.hasNext()){
				...
				if(!key.equals("counter")){
					...
					if(child.getInt("id") == id){
						Firebase base = new Firebase("https://scorching-inferno-6707.firebaseio.com/post/");
	...
	public boolean addComment(@WebParam(name = "id") int id, @WebParam(name = "nama") String nama, @WebParam(name = "email") String email, @WebParam(name = "komentar") String komentar) {
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		Date date = new Date();
		try {			
			JSONObject json = new JSONObject(readUrl("https://scorching-inferno-6707.firebaseio.com/komentar/.json"));
			int counter = json.getInt("counter") + 1;
			Firebase base = new Firebase("https://scorching-inferno-6707.firebaseio.com/komentar/");
			Comment comment = new Comment(id, nama, email, dateFormat.format(date), komentar);
	...
	public ArrayList<Comment> listComment(@WebParam(name = "id") int id) {
		...
		try {			
			...
			while(keys.hasNext()){
				...
				if(!key.equals("counter")){					
					JSONObject child = json.getJSONObject(key);
					if (child.getInt("id") == id) {
						Comment comment = new Comment(child.getInt("id"), child.getString("nama"), child.getString("email"), child.getString("tanggal"), child.getString("komentar"));
		...
		try {					
			...
			while(keys.hasNext()){
				...
				if(!key.equals("counter")){
					...
					if(child.getInt("id") == id){
						Firebase base = new Firebase("https://scorching-inferno-6707.firebaseio.com/komentar/");
						comment = new Comment(child.getInt("id"), child.getString("nama"), child.getString("email"), child.getString("tanggal"), child.getString("komentar"));
						break;
					}
				}
			}			
